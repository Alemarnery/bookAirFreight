...
const Form = () => {
  ...
  if (submit) {
    ...
    const waitDay = useMemo(
      () =>
        Math.floor(Math.random() * (END_MAX_DAY - END_MIN_DAY) + END_MIN_DAY),
      []
    );
  ...
  const RenderedTable = () => {
    ...
    return (
      <table>
        ...
        <tbody>
          <tr>
            <td>Start Day {startDay}</td>
            <td>US $</td>
          </tr>
          <tr>
            <td>End Day {endDay}</td>
            <td>
